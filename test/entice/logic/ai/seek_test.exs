defmodule Entice.Logic.SeekTest do
  ...
  setup do
    npc_init_coord =  %Vector2D{x: 10, y: 10}
    {:ok, npc_eid, npc_pid} = Npc.spawn(TestMap, "Dhuum", :dhuum, %Position{coord: npc_init_coord}, seeks: true)
    ...
    simulate_movement_update(player_pid, %Position{coord: %Vector2D{x: 1, y: 2}}, %Movement{goal: %Vector2D{x: 3, y: 4}})
  ...
  test "update same entity", %{npc_entity: pid} do
    simulate_movement_update(pid, %Position{coord: %Vector2D{x: 1, y: 2}}, %Movement{goal: %Vector2D{x: 3, y: 4}})
  ...
  test "update no target, entity close enough to aggro", %{npc_entity: npc_pid, player_eid: player_eid, player_entity: player_pid} do
    ...
    mover_coord = %Vector2D{x: 14, y: 10}
  ...
  test "update no target, entity too far to aggro", %{npc_entity: npc_pid, player_entity: player_pid} do
    simulate_movement_update(player_pid, %Position{coord: %Vector2D{x: 460, y: 910}}, %Movement{goal: %Vector2D{x: 0, y: 0}})
  ...
  test "update has target, entity is not current target", %{npc_entity: npc_pid, player_eid: player_eid} do
    ...
    simulate_movement_update(other_player_pid, %Position{coord: %Vector2D{x: 1, y: 2}}, %Movement{goal: %Vector2D{x: 3, y: 4}})
    ...
    simulate_movement_update(other_player_pid, %Position{coord: %Vector2D{x: 11, y: 11}}, %Movement{goal: %Vector2D{x: 0, y: 0}})
    assert {:ok, %Seek{target: ^player_eid}} = Entity.fetch_attribute(npc_pid, Seek)
  end
  ...
  %{npc_entity: npc_pid, player_eid: player_eid, player_entity: player_pid, npc_init_coord: npc_init_coord} do
    ...
    simulate_movement_update(player_pid, %Position{coord: %Vector2D{x: 30, y: 15}}, %Movement{goal: %Vector2D{x: 0, y: 0}})
    assert {:ok, %Seek{target: nil}} = Entity.fetch_attribute(npc_pid, Seek)
  ...
  test "update has target, entity is current target, entity does not escape", %{npc_entity: npc_pid, player_eid: player_eid, player_entity: player_pid} do
    ...
    new_player_coord = %Vector2D{x: 14, y: 15}
    simulate_movement_update(player_pid, %Position{coord: new_player_coord}, %Movement{goal: %Vector2D{x: 0, y: 0}})
    assert {:ok, %Seek{target: ^player_eid}} = Entity.fetch_attribute(npc_pid, Seek)
